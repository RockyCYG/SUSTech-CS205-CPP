...
class Matrix {
   ...
   public:
    ...
    //实现的是weight *
    // Matrix(使用ikj访存优化)并且加上偏移量bias，flag变量来确定此次要不要做relu操作
    void mul(T* weight,
             T* bias,
             size_t row,
             size_t col,
    ...
    //使用OpenBLAS计算矩阵乘法weight *
    // Matrix并且加上偏移量，flag变量来确定此次要不要做relu操作
    void mul_openblas(T* weight,
                      T* bias,
                      size_t row,
                      size_t col,
                      Matrix<T>& out,
    ...
    for (size_t i = 0; i < row; i++) {
        for (size_t k = 0; k < col; k++) {
            ...
            for (size_t j = 0; j < this->cols; j++) {
                out.data[i * this->cols + j] +=
                    t * this->data[k * this->cols + j];
            }
        }
    }
...
// 使用OpenBlas库计算矩阵乘法
template <typename T>
void Matrix<T>::mul_openblas(T* weight,
                             T* bias,
                             size_t row,
                             size_t col,
                             Matrix<T>& out,
    ...
    out.data = new T[row * this->cols];
    memset(out.data, 0, sizeof(T) * row * this->cols);
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, row, this->cols, col,
                1, weight, col, this->data, this->cols, 0, out.getData(),
                this->cols);
    for (size_t channel = 0; channel < out.rows; channel++) {
        for (size_t y = 0; y < out.cols; y++) {
            out[channel][y] += bias[channel];
...
Matrix<T> Matrix<T>::operator*(const Matrix<T>& m) const {
    ...
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, this->rows, m.cols,
                this->cols, 1, this->data, this->cols, m.data, m.cols, 0,
                mul.data, m.cols);
    return mul;
}
