...
void imgToMat(cv::Mat &img, Matrix<float> &out, size_t kernel_size, size_t paddings, size_t stride) {
        ...
        for (size_t j = 0, cnt2 = 0; cnt2 < size; j += stride, col++, cnt2++) {
            ...
            for (size_t r = i; r < i + kernel_size; r++) {
                for (size_t c = j; c < j + kernel_size; c++, row++) {
                    if (paddings > 0) {
                        if (r == 0 || r == in_size + 1 || c == 0 || c == in_size + 1) {
                            out[row][col] = 0;
                        } else {
                            out[row][col] = R.at<float>(r - paddings, c - paddings);
                        }
                    } else {
                ...
                for (size_t c = j; c < j + kernel_size; c++, row++) {
                    if (paddings > 0) {
                        if (r == 0 || r == in_size + 1 || c == 0 || c == in_size + 1) {
                            ...
                            out[row][col] = G.at<float>(r - paddings, c - paddings);
                        }
                    } else {
                ...
                for (size_t c = j; c < j + kernel_size; c++, row++) {
                    if (paddings > 0) {
                        if (r == 0 || r == in_size + 1 || c == 0 || c == in_size + 1) {
                            ...
                            out[row][col] = B.at<float>(r - paddings, c - paddings);
                        }
                    } else {
                    ...
                    for (size_t c = j; c < j + kernel_size; c++, row++) {
                        if (paddings > 0) {
                            if (r == 0 || r == in_size + 1 || c == 0 || c == in_size + 1) {
                                ...
                                out[row][col] = in[channel][(r - paddings) * in_size + c - paddings];
                            }
                        } else {
...
void convAndRelu(Matrix<float> &in, conv_param &conv_p, Matrix<float> &out) {
    in.mul(conv_p.p_weight, conv_p.p_bias, conv_p.out_channels,
           conv_p.in_channels * conv_p.kernel_size * conv_p.kernel_size, out, true);
}
    ...
    for (size_t channel = 0; channel < in.getRows(); channel++) {
        for (size_t row = 0; row < size; row += 2) {
            for (size_t col = 0; col < size; col += 2) {
                float a = in.getData()[channel * in.getCols() + row * size + col];
                float b = in.getData()[channel * in.getCols() + row * size + col + 1];
                float c = in.getData()[channel * in.getCols() + row * size + col + size];
                float d = in.getData()[channel * in.getCols() + row * size + col + size + 1];
                //取2*2方块中的最大值
...
void fullyConnected(Matrix<float> &in, fc_param &fc_p, Matrix<float> &out) {
    in.mul(fc_p.p_weight, fc_p.p_bias, fc_p.out_features, fc_p.in_features, out, false);
}